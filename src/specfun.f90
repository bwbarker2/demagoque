...
!
!    Renamed to not conflict with intrinsic function.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
  ...
  if ( .not. bool1 .and. bool2 ) then
    ...
    n1 = int(c0 * x1 * t - one)   ! explicit typecast added to avoid compiler warning.
    if ( n1 < nmax ) then
      n1 = nmax
    end if
...
!
  ...
  ren = k  ! Does nothing, but added to avoid 'unused dummy variable' compiler warning
...
!
  ...
  real ( kind = 8 ) b(nb)
  real ( kind = 8 ) const
!  real ( kind = 8 ) r8_gamma  !module now, so no declaration needed
  real ( kind = 8 ) em
  real ( kind = 8 ) empal
  real ( kind = 8 ) emp2al
  real ( kind = 8 ) en
